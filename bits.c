...
int allEvenBits(int x) {
    int tester = 170;
    tester = tester | (tester << 8);
    tester = tester | (tester << 16);
...
int reverseBits(int x) {
    ...
    mask_d = ((mask_d<<8) | 255); //(0xff)
    mask_d = (mask_d | (mask_d<<16));
    
    mask_e = (mask_e | (mask_e<<8));
    mask_e = (mask_e | 00);
    
    x = ((x>>1)&(mask_a)) | ((x & mask_a)<<1);
    x = ((x>>2)&(mask_b)) | ((x & mask_b)<<2);
    x = ((x>>4)&(mask_c)) | ((x & mask_c)<<4);
    x = ((x>>8)&(mask_d)) | ((x & mask_d)<<8);
    x = ((x>>16)&(mask_e)) | ((x & mask_e)<<16);
    return x;
}
/*
...
int satAdd(int x, int y) {
    ...
    int isOverf = (xSign ^ addSign) & (ySign ^ addSign);
    int final = ( (~isOverf) & add ) | ( isOverf & ((1<<31)+addSign) );
    return final;
}
/*
 * Extra credit
